<?php
/*
Plugin Name: شمارنده ذکرهای بهشتی
Plugin URI : http://wp-master.ir
Author: wp-master.ir
Author URI: http://wp-master.ir
Description: شمارنده ذکرهای بهشتی | امکان ثبت هر نوع ذکر و ختم توسط کاربران سایت
Version: 0.9.1
...
function _salavatcounter_lang()
{
     ...
     * Plugin DB upgrade
     */
    $db_ver = get_option('salavat_counter_db_version', 1);
    if ($db_ver == 1) {
        global $wpdb;
        //create tables
        $salavatcounter = $wpdb->prefix . 'salavatcounter';
        $wpdb->query("ALTER TABLE $salavatcounter ADD COLUMN `other_options` TEXT NULL");
        update_option('salavat_counter_db_version', 2);
    }
...
{
    add_option('salavat_counter_redirect_after_activation_option', true);
...
{
    if (get_option('salavat_counter_redirect_after_activation_option', false)) {
        delete_option('salavat_counter_redirect_after_activation_option');
        exit(wp_redirect(admin_url('options-general.php?page=salavatcounter-options')));
    }
    ...
    if (isset($_GET['salavat-counter-js']) && trim($_GET['salavat-counter-js']) != '') {
        ...
        $other_options = json_decode($salavat->other_options, 1);
        if (!is_array($other_options)) {
            $other_options = [];
        }
        $has_other_zekr = false;
        $zekr = 'صلوات';
        if (isset($other_options['zekr']) && !empty($other_options['zekr'] && $other_options['zekr'] != 'صلوات')) {
            $extra_class = ' salavatacounter-has-other-zekr ';
            $has_other_zekr = true;
            $zekr = $other_options['zekr'];
        }
        ...
        if ($has_other_zekr) {
            $other_style_file = _salavatcounter_DIR . 'css' . DIRECTORY_SEPARATOR . 'styles' . DIRECTORY_SEPARATOR . 'other_zekr' . '.css';
            if (file_exists($other_style_file)) {
                $style .= file_get_contents($other_style_file);
        ...
        if (!$has_other_zekr) {
            $html .= "<img class='salavat-ajax-loader salavat-counter-ajax-loader' src='" . _salavatcounter_PATH . "css/img/ajax-loader.gif'>";
        }

        $html .= "
				<div class='" . $extra_class . $class . "'>
					<span class='salavatacounter-count'>" . $salavat->salavat . "</span>
					<span class='salavatcounter-add-salavat' data-id='" . $salavat->id . "'>" . $zekr . "</span>
				</div>";
        ...
        */
        ...
        <?php
        echo 'this_counter_placeholder.html(\'<img class="salavat-counter-ajax-loader" src="' . _salavatcounter_PATH . 'css/img/ajax-loader.gif">\');';
        ?>
        $.ajax({
...
function salavat_counter_make_form($echo = true, $shortcode = false, $id = false)
{
    ...
    if (!$id) {
        ...
        if (!$row) {
            if ($echo) echo 'ذکر مورد نظر گویا حذف شده است'; else  return 'ذکر مورد نظر گویا حذف شده است';
  ...
  <div class="salavat-counter ' . implode(' ', $class) . '">';
    ...
    $html .= '
    <p><span class="awaiting-mod"><span class="pending-count sc-padding-count"> ' . $counter . '</span></span><span class="sc-till-now">' . sprintf(__('%s was saied till now!', $txt_domain), $zekr) . '</span></p>
    ';
    if ($img_show) {
        $html .= '    <p>
    ...
    $html .= '
    <p>
      <button class="sc-say-salavat" data-id="' . $id . '"><img class="ajax_loader_gif" src="' . plugin_dir_url(__FILE__) . '/css/img/ajax-loader.gif" style="display:none;">&nbsp;&nbsp;&nbsp;' . sprintf(__('say 1 %s', $txt_domain), $zekr) . '&nbsp;&nbsp;&nbsp;<img class="ajax_loader_gif" src="' . plugin_dir_url(__FILE__) . '/css/img/ajax-loader.gif" style="display:none;"></button>
    </p>
...
class widget_salavat_shomar extends WP_Widget
{
    function __construct()
    {
        ...
        parent::__construct(false, __('salavat counter', 'salavatcounter'), $widget_ops);
    }
    ...
    function form($instance)
    {
        ...
        ?>
        <p>
            <?php _e('title', 'salavatcounter'); ?> ::: <input type="text"
        ...
        <p>
            ...
            <select name="<?php echo $this->get_field_name('salavat_id'); ?>"
                    id="<?php echo $this->get_field_id('salavat_id'); ?>">
                <?php
...
add_action('widgets_init', 'salavat_counter_widgets_init');
function salavat_counter_widgets_init()
{
    return register_widget("widget_salavat_shomar");
}
